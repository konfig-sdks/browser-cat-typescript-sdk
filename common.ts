...
import { readableStreamToString, BrowserCatError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new BrowserCatError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new BrowserCatError(e, e.response?.data)
                }
                if (innerError instanceof BrowserCatError) {
                    // Got "BrowserCatError" from the above try block
                    throw innerError;
                }
